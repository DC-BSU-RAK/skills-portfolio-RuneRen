...
def recursive_scale(widget, title_size, label_size, button_size, entry_size):
    if isinstance(widget, tk.Label):
        ...
        if "ARITHMETIC QUEST" in text or "GAME OVER" in text:
...
def start_timer():
    ...
    def countdown():
        ...
        if time_left > 0:
            ...
            remaining_width = int(bar_width * (time_left / full_time))
            timer_bar.coords(timer_bar_rect, 0, 0, remaining_width, bar_height)

            percentage = time_left / full_time
            if percentage > 0.5:
                color = "#00FF00"
            elif percentage > 0.2:
                color = "#FFFF00"
            ...
            play_wrong_sound()
            messagebox.showinfo("TIME UP", "Out of time!")
            nextQuestion()
    countdown()
...
def displayProblem():
    ...
    global timer_bar, timer_bar_rect, bar_width, bar_height
...
#main window
root = tk.Tk()
root.title("Retro Arithmetic Quest")
